...
llm_build_deepseek4::llm_build_deepseek4(const llama_model & model, const llm_graph_params & params) :
    ...
    if (is_mtp) {
        ...
        GGML_ASSERT(layer.nextn.e_proj && "DeepSeek V4 MTP block missing nextn.e_proj");
        GGML_ASSERT(layer.nextn.h_proj && "DeepSeek V4 MTP block missing nextn.h_proj");
        GGML_ASSERT(layer.nextn.enorm  && "DeepSeek V4 MTP block missing nextn.enorm");
        GGML_ASSERT(layer.nextn.hnorm  && "DeepSeek V4 MTP block missing nextn.hnorm");

        auto inp = std::make_unique<llm_graph_input_embd_h>(n_embd * n_hc);
        inp->tokens = ggml_new_tensor_1d(ctx0, GGML_TYPE_I32, n_tokens);
        ggml_set_input(inp->tokens);
        inp->embd = ggml_new_tensor_2d(ctx0, GGML_TYPE_F32, n_embd * n_hc, n_tokens);
        ggml_set_input(inp->embd);
        inp->h = ggml_new_tensor_2d(ctx0, GGML_TYPE_F32, n_embd * n_hc, n_tokens);
        ggml_set_input(inp->h);
        ggml_set_name(inp->h, "mtp_h_input");

        inp_tokens = inp->tokens;
        ggml_tensor * tok_embd = ggml_get_rows(ctx0, model.tok_embd, inp_tokens);
        tok_embd = build_norm(tok_embd, layer.nextn.enorm, nullptr, LLM_NORM_RMS, il);
        tok_embd = ggml_mul_mat(ctx0, layer.nextn.e_proj, tok_embd);
        tok_embd = ggml_reshape_3d(ctx0, tok_embd, n_embd, 1, n_tokens);
        tok_embd = ggml_repeat_4d(ctx0, tok_embd, n_embd, n_hc, n_tokens, 1);

        ggml_tensor * h_embd = ggml_reshape_3d(ctx0, inp->h, n_embd, n_hc, n_tokens);
        h_embd = build_norm(h_embd, layer.nextn.hnorm, nullptr, LLM_NORM_RMS, il);
        h_embd = ggml_reshape_2d(ctx0, h_embd, n_embd, n_hc * n_tokens);
        h_embd = ggml_mul_mat(ctx0, layer.nextn.h_proj, h_embd);
        h_embd = ggml_reshape_3d(ctx0, h_embd, n_embd, n_hc, n_tokens);

        inpL = ggml_add(ctx0, tok_embd, h_embd);
        cb(inpL, "mtp_inp", il);
        res->add_input(std::move(inp));
    } else {
        inpL = build_inp_embd(model.tok_embd);
        inp_tokens = res->t_inp_tokens;
    ...
    for (int il = il_begin; il < il_end; ++il) {
        ...
        if (compress_ratio == 0) {
            ...
            if (!is_prefill) {
                ...
                if (attn_mask->type != GGML_TYPE_F32) {
    ...
    const auto & head_layer = model.layers[is_mtp ? n_main_layers : 0];
    ggml_tensor * cur = dsv4_hc_head(ctx0, inpL,
            is_mtp ? head_layer.nextn.hc_head_fn    : model.output_hc_fn,
            is_mtp ? head_layer.nextn.hc_head_scale : model.output_hc_scale,
            is_mtp ? head_layer.nextn.hc_head_base  : model.output_hc_base,
